 ...
 *  2. The "a" slides right over "im" (erasing them)
...
export default function AnimatedAimLogo({
  ...
  const aTranslateX =
    phase === "sliding-right" ? slideDistance : 0;
  ...
  return (
    ...
    >
      ...
      {!isIcon && (
        ...
        >
          {/* "i" and "m" — hidden once "a" starts sliding */}
          <g
            style={{
          ...
          {/* "a" — slides right then back */}
          <g
            style={{
              transform: `translateX(${aTranslateX}px)`,
      ...
      {/* Icon mark — instant swap, no fade */}
      {isIcon && (
        <svg
