 ...
 *
 * Sequence:
 *  1. Full "aim" word fades in letter-by-letter (a → i → m)
 *  2. After a pause, "im" slides out and fades
 *  3. The "a" scales/shifts to become a standalone icon mark
 */
export default function AnimatedAimLogo({
  ...
  const [phase, setPhase] = useState<"full" | "icon">("full");
  ...
  // Full logo: viewBox covers the entire "aim" word
  // Icon: viewBox zooms into just the "a" letterform
  const fullBox = "172 210 312 153";
  const iconBox = "165 205 155 165";

  return (
    <svg
      viewBox={phase === "full" ? fullBox : iconBox}
      width={phase === "full" ? 64 : 32}
      height={phase === "full" ? 31 : 32}
    ...
    >
      {/* "a" — always visible */}
      <g
        className={`${fillClass} transition-all duration-700 ease-in-out`}
        style={{
          transform: phase === "icon" ? "translate(0, 0)" : undefined,
        }}
      >
        {/* "a" — circle bowl */}
      ...
      {/* Counter dot inside "a" */}
      <circle
        cx="242"
        cy="294.5"
        r="20"
        className={`${counterClass} transition-all duration-700`}
      />
      ...
      <path
        ...
        className={`${fillClass} transition-all duration-500 ease-in-out`}
        style={{
          opacity: phase === "icon" ? 0 : 1,
          transform: phase === "icon" ? "translateX(20px)" : "translateX(0)",
        }}
      />
      ...
      <path
        ...
        style={{
          opacity: phase === "icon" ? 0 : 1,
          transform: phase === "icon" ? "translateX(30px)" : "translateX(0)",
        }}
      />
    </svg>
  );
